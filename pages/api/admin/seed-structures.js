...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const structure of payoutStructures) {
      ...
      createdPayoutStructures.push(payoutStructure);
    }

    return res.status(200).json({ 
      success: true, 
      message: 'Tournament structures created successfully',
      blindStructure,
      payoutStructures: createdPayoutStructures
    });
  } catch (error) {
    console.error('Error creating tournament structures:', error);
    return res.status(500).json({ 
      success: false, 
      message: 'Failed to create tournament structures', 
