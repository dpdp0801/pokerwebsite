...
export function ensureDataDirectory() {
  ...
  console.log(`Checking data directory: ${dataDirectory}`);
  ...
  // Check if blind structure file exists
  const blindStructurePath = path.join(dataDirectory, 'blindStructure.json');
  if (!fs.existsSync(blindStructurePath)) {
    console.log('Blind structure file does not exist, creating it');
    ...
    fs.writeFileSync(blindStructurePath, JSON.stringify(defaultBlindStructure, null, 2));
  }

  // Check if payout structures file exists
  const payoutStructuresPath = path.join(dataDirectory, 'payoutStructures.json');
  if (!fs.existsSync(payoutStructuresPath)) {
    console.log('Payout structures file does not exist, creating it');
    // Create default payout structures
    const defaultPayoutStructures = [
      {
        name: "2-10 Players",
        minEntries: 2,
        maxEntries: 10,
    ...
    fs.writeFileSync(payoutStructuresPath, JSON.stringify(defaultPayoutStructures, null, 2));
  }
