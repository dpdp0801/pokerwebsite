...
export const shouldShowPayouts = (blindStructureData, currentSession) => {
  ...
  if (break2Index !== -1 && levelIndex >= break2Index) {
    return true;
  }
  
  // Alternative: If Break 2 isn't explicitly marked, check for levels with REG_CLOSE action
  ...
  if (regCloseIndex !== -1 && levelIndex >= regCloseIndex) {
    return true;
  }
