...
/**
 * Format only the time part of a date string (expected ISO format from DB)
 */
export const formatTimeOnly = (dateTimeString) => {
  if (dateTimeString) { // Expect ISO string (likely UTC)
    try {
      // Parse the ISO string (which includes timezone info or is UTC)
      // and format it according to the browser's local time.
  ...
  return 'TBD'; // If dateTimeString is missing
};
