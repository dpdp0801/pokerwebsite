...
/**
 * Format only the time part from HH:MM string
 */
export const formatTimeOnly = (timeString_HH_MM) => {
  if (!timeString_HH_MM || typeof timeString_HH_MM !== 'string') return 'TBD'; 
  try {
    // Parse HH:MM string into a Date object (relative to today for formatting)
    const parsedTime = parse(timeString_HH_MM, 'HH:mm', new Date());
    return format(parsedTime, 'h:mm a');
  } catch (e) {
    console.error('Error formatting time string:', timeString_HH_MM, e);
    return 'Invalid time';
  }
};
