  ...
  const updatePlayerStatus = async (registrationId, newStatus, isRebuy = false) => {
    try {
      if (newStatus === 'ELIMINATED') {
        ...
        console.log(`Moving player to waitlist: ${registrationId}`);
        const response = await fetch(`/api/player-status`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
        ...
        if (response.ok) {
          console.log('Waitlist response:', await response.clone().json());
          toast({
            title: "Status Updated",
            description: "Player has been moved to waitlist",
          });
          
          // Refresh the session data with a slight delay to ensure DB update is complete
          setTimeout(async () => {
            await fetchSessionData();
          }, 500);
          return true;
