...
export const usePlayerService = (fetchSessionData) => {
  ...
  const seatFromWaitlist = async (registration) => {
    try {
      const response = await fetch(`/api/registration/${registration.id}/confirm`, {
        method: 'POST'
      });
      ...
      if (data.success) {
        ...
        await updatePlayerStatus(registration.id, 'CURRENT');
        toast({
          title: "Player Seated",
